...
public class WorkoutLog extends Application implements SessionManager, ClickListener, ValueChangeListener
{
	private static final long serialVersionUID = -803789169360367310L;
	private Table table;
	...
	/**
	 * This column generator transforms trainingType column to human readable form (instead of displaying the identifier
	...
	private ColumnGenerator traininTypeReadOnlyColumn = new ColumnGenerator()
	{
		 ...
		 * 
		 */
		private static final long serialVersionUID = 2928854646820591026L;

		public Component generateCell(Table source, Object itemId, Object columnId)
		{
			Label l = new Label();
			Object id = table.getContainerProperty(itemId, "trainingType").getValue();
			if (id == null)
			{
				l.setValue(" - ");
			}
			...
			{
				Type trainingtype = (Type) getSession().get(Type.class, (Serializable) id);
	...
	/**
	 * We are using session-per-request pattern with Hibernate. By using Vaadin's transaction listener we can easily
	 * ensure that session is closed on each request without polluting our program code with extra logic.
	 */
	...
	private void buildView()
	{
		...
		((AbstractLayout) w.getContent()).setMargin(false);
		...
		mainLayout.addComponent(new Label("This is a Vaadin example " + "application that is using Hibernate to persist its model" + " objects. Check help for more details."));
	...
	protected void populateAndConfigureTable()
	{
		...
		table.addActionHandler(new Handler()
		{
			 ...
			 * 
			 */
			private static final long serialVersionUID = 5421223220123250288L;
			...
			public void handleAction(Action action, Object sender, Object target)
			{
				...
				{
					Workout w = (Workout) getSession().get(Workout.class, (Serializable) target);
					deleteRun(w);
				}
				else if (action == edit)
				{
	...
	protected void loadWorkouts()
	{
		...
		if (actionButtonColumnMode == ACTION_MODE_GENERATOR)
		{
			...
			table.addGeneratedColumn("actions", new ColumnGenerator()
			{
				 ...
				 * 
				 */
				private static final long serialVersionUID = 1063785280631257728L;
				...
				public Component generateCell(Table source, final Object itemId, Object columnId)
				{
					...
					b.addListener(new ClickListener()
					{
						...
						{
							editRun((Workout) ((EntityItem) cont.getItem(itemId)).getPojo());
						}
					});
					...
					del.addListener(new ClickListener()
					{
						...
						public void buttonClick(ClickEvent event)
						{
							deleteRun((Workout) ((EntityItem) cont.getItem(itemId)).getPojo());
						}
					});
	...
	/**
	 * Saves changes made to Workout object to Hibernate Session. Note that run is most likely detached due
	 * session-per-request patterns so we'll use merge. Actual database update will happen by Vaadin's transaction
	 * listener in the end of request.
	 * 
	 * If one wanted to make sure that this operation will be successful a (Hibernate) transaction commit and error
	 * checking ought to be done.
	 * 
	 * @param run
	 */
	...
	/**
	 * Delete given Workout object from Table. Table will delegate deletion to its container.
	 * 
	 * @param run
	 */
	...
	public void valueChange(ValueChangeEvent event)
	{
		...
		else if (event.getProperty() == actionButtonMode)
		{
			...
			populateAndConfigureTable();
			loadWorkouts();
			((AbstractOrderedLayout) old.getParent()).replaceComponent(old, table);
			mainLayout.setExpandRatio(table, 1);
			table.setSizeFull();
